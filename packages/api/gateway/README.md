...
A strict unary descriptor with `invocation.kind: 'view'` uses that physical WebSocket as its receiver authority. Gateway privately follows Session Controller's read-only current-selection snapshot and exposes no public binding setter. The sole Host resolver validates the connection's displayed Session and returns a child Cordis Context; the descriptor carries no wire identity. Rebinding clears the old Context before resolving the new Session and cancels active view calls. HTTP, in-process, stale-connection, and unbound calls fail before business execution. The [Host-owned Web view Context decision](../../../.agents/notes/implemented/architecture/2026-08-31-host-owned-web-view-context.md) owns the security and lifecycle rationale.
...
Each unary call validates positional inputs and constructs the descriptor's exact named `args`. Direct and caller-selected Context methods use `ctx.connection.rpc.call('/api', endpoint, ...)`; view receiver methods first synchronize the current Session binding, then use the shared Gateway WebSocket without adding an identity field. A generated stream method returns an `AsyncIterable` and opens one logical stream through an in-process Connection carrier when available, otherwise through the same WebSocket. Generated cancellation-aware methods accept a final optional `AbortSignal`; the Client combines it with the contribution mount lifetime before invoking the carrier. Unary results and every stream item are validated before reaching application code. Withdrawing a contribution removes its descriptors and methods together, aborts in-flight calls and streams, and makes retained method handles reject.
