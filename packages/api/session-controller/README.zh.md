...
Gateway 在内部观察 Client 的只读 `sessions.list.current` 快照，并把它用于物理 Web view 绑定；不存在可直接替换该绑定的 Client Remote 方法。Host 通过 `sessionQuery` 验证该会话，且不会激活 Agent，并只在 Host 创建的子上下文中以 `viewSessionId` 公开其 id。导航、重连和 dispose 会更新或清除绑定；业务 Remote payload 无法提供该绑定。
