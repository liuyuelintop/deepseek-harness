...
Client 还会把 `sessions.list.current` 投影到 Gateway 的物理 Web view 绑定。Host 通过 `sessionQuery` 验证该会话，且不会激活 Agent，并只在 Host 创建的子上下文中以 `viewSessionId` 公开其 id。导航、重连和 dispose 会更新或清除绑定；业务 Remote payload 无法提供该绑定。
