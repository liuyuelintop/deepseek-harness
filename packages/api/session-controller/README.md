...
Gateway privately observes the Client's read-only `sessions.list.current` snapshot for its physical Web view binding; no Client Remote method can replace that binding directly. The Host validates that Session through `sessionQuery` without activating an Agent and exposes its id only as `viewSessionId` on the Host-created child Context. Navigation, reconnect, and disposal update or clear the binding; business Remote payloads cannot supply it.
