...
The Client also projects `sessions.list.current` into Gateway's physical Web view binding. The Host validates that Session through `sessionQuery` without activating an Agent and exposes its id only as `viewSessionId` on the Host-created child Context. Navigation, reconnect, and disposal update or clear the binding; business Remote payloads cannot supply it.
